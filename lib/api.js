...
/**
 * Tests to see if the url is a valid Axosoft url via calling the /version api.
 * A boolean saying if the url is valid or not is passed in as the callback result.
 *
 * @param	function	callback	Callback function.
 */
function verifyUrl(callback) {
	rest.get(_axosoft_api_base_url + '/version')
		.on('complete', function(data, response) {
			var isVerified = !!response && (response.statusCode === 200 || response.statusCode === 201);
